...
# This cell is not included in the final rendered document.
# It is usually used to add initial code setup, such as loading libraries or setting global options.
knitr::opts_chunk$set(echo = TRUE)  # Actual code will be shown in the output alongside its results
```
...
sum(3, 6, 8, NA, na.rm=T) # removing na in command before calculation
...
2.  We canuse the names() function to assign names to all values in an existing list
...
```{r}
library(datasets)
data(iris)  # loads a dataset that is included in a package (like datasets) into oru R env
...
In any dataframe (as with pretty much any table structure) all values in a column must be of the same type, for example, a name column will always be of character type. To think about this in terms of *R*, this means that every column in a data frame will be a **vector**.
...
```{r}
Sepal.Columns <- grepl("Sepal", names(iris))  # Columns containing 'Sepal'
Virginica.Rows <- iris$Species == "virginica"  # Rows for the 'virginica' species
result <- iris[Virginica.Rows, Sepal.Columns]  # Subset of the data filtering with the criteria from the previous two lines
...
Complete the exercises to check the understanding of basic R concepts.

## Exercise 1: Basic Data Types in R

Create three variables in R: one numeric, one logical, and one character. Check their types using the `class()` function.

```{r}
# like python don't need to specify the types, just assign object and R assigns it a ttype
x <- 3
y = TRUE
z = 'chartest'
class(x)
...
## Exercise 2: Type Conversion in R

Convert a numeric value to an integer and a character type. Observe and explain what happens when you try to perform mathematical operations on each type.
...
# convert to integer
xint <- as.integer(num_val)
class(xint)

# convert to character
xchar <- as.character(num_val)#
class(xchar)

# perform operaton
sum_int <- xint + 10
# sum_char <- xchar + 10 # this will produce and error
...
## Exercise 3: Numeric Vectors

Create a numeric vector with both integer and double values. Use the `typeof()` function to determine the type of each element in the vector.

```{r}

vec1 <- c(1.9, 2, 4, 6, 8,6.1, 10L)
class(vec1)
typeof(vec1)
sapply(vec1, typeof)

# solution
num_vector <- c(1L, 2.5, 3,  4.7)

# determine the type of each element
typeof(num_vector)

# all numberc type_sum(use c letetr ton crate vecotors)

```

## Exercise 4: Vector Operations

Create a vector containing the numbers from 1 to 10. Write R code to calculate the sum, mean, and standard deviation of the vector.
...
## Exercise 5: Lists in R

Create a list containing at least one element of each basic data type (numeric, logical, character). Access and print the second element of the list.

```{r}

list1 <- list(1, 3, TRUE, 'string1')
print(list1)
list1[[1]]

#solution
my_list <- list(42, TRUE, 'R programming')

second_element <- my_list[[2]] # index starts from 1 and not from zero
second_element

```

## Exercise 6: Functions in R

Write a function in R that takes a vector as input and returns a list with the following components: the length of the vector, the sum of its elements, and the product of its elements.
...
# example usage
example_vec<- 1:5
vector_summary(example_vec)

```

## Exercise 7: Creating Data Frames

Create a data frame with three columns: Name (character), Age (numeric), and IsStudent (logical). Add at least five rows of data.

```{r}
Name<-c('Jim', 'John', 'James', 'Catherine', 'Suzanne')
Age<-c(35, 16, 17, 21, 22)
IsStudent<-c(TRUE, FALSE, FALSE, TRUE, TRUE)
df1<-data.frame(Name, Age, IsStudent)
df1


# Solution
# don't need to imoport pandas as we do in python
df9 <- data.frame(
  Name = c("Alice", "Bob", "Charlie", "David", "Eva"),
  Age = c(23, 30, 22, 35, 28),
...
## Exercise 8: Modifying Data Frames

Write R code to add a new column to the data frame created in Exercise 7, which calculates the age of each person in months.

```{r}
df1$AgeinMonths <-(df1$Age * 12)

df1

# Solution
# dollar sign access feature and using caretign new column name
df9$AgeInMonths <- df9$Age * 12
...
## Exercise 9: Subsetting Data Frames

Use the `subset()` function to filter the data frame for rows where `IsStudent` is `TRUE`.

```{r}

studentdf2 <- subset(df1, IsStudent == TRUE )
studentdf2

# solution 9
# subset allows us create subset of the data based ont he condition yuo state...
studentdf <- subset(df9, IsStudent == TRUE )
studentdf

```

## Exercise 10: Calculating Summary Statistics

Write a script to calculate the average age of students in the data frame.

```{r}
average_age <- mean(studentdf2$Age)
print(average_age)
